...
```
  ...
  Ans : 
      Create a binary variable mgp01 , find the median() function, and combine the binary variable mpg01 and Auto as a Single data set 
...
```
  ...
  Ans : As we have observed from the above plot that  `mpg01` has a negative association with `Weight`, `Cylinders`, `Displacement`, and `Horsepower`.
...
```
  Ans : We have splitted the data into training set and a test set
        The training data contains 210 records
        The test data contains 182 records
